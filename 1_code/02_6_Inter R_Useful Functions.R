...
is.list(c(1, 2 ,3)) # FALSE, it's a vector, not a list.
...
# With grepl() funtion, e.g, we can determine which animal has an "a" in their name. The 1st argument is the pattern, 2nd argument is the character vector where matches are sought.
...
grep(pattern = "a$", x = animals)
# E.g, query and manipulate a character vector of email addresse
...
# The 3rd animal name "impala" was replaced with "impola" instead of "impolo", because as soon as sub() finds the matches, it replaces it with the replacement argument, then immediately stop looing. 
