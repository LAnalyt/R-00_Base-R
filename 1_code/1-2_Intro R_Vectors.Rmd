---
title: "INTRODUCTION TO R: VECTORS"
source: DataCamp
---

*Vectors* are one-dimension *arrays* that can hold numeric data, character data, or logical data.

### 2.1 Create a vector

A vector can be created with the combine function `c()`. Vector elements are separated by a comma.
```{r}
numeric_vector <- c(1, 2, 3)
character_vector <- c("a", "b", "c")
boolean_vector <- c(TRUE, FALSE, TRUE)
```
For example, make an analysis for a gamble games in Las Vegas. Crete a data vector for poker game:

-   On Monday you won $140
-   Tuesday you lost $50
-   Wednesday you won $20
-   Thursday you lost $120
-   Friday you won $240
```{r}
poker_vector <- c(140, -50, 20, -120, 240) # negative number for losing money.
```
Similarly, create a data vector for roulette game:

-   On Monday you lost $24
-   Tuesday you lost $50
-   Wednesday you won $100
-   Thursday you lost $350
-   Friday you won $10
```{r}
roulette_vector <- c(-24, 50, 100, -350, 10)
```

### 2.2 Naming a vector

It is important to have a clear view on the data that you are using. Understanding what each element refers to is essential. Give a name to the elements of a vector with the `names()` function:
```{r}
some_vector <- c("John Doe", "poker player")
names(some_vector) <- c("Name", "Profession")
```
See how it appears with names:
```{r}
some_vector
```
Continue with the Las Vegas exercise, assign the days as *names*:
```{r}
names(poker_vector) <- c("Monday","Tuesday","Wednesday","Thursday","Friday")
names(roulette_vector) <- c("Monday","Tuesday","Wednesday","Thursday","Friday")
```
Print out to see the results:
```{r}
poker_vector
roulette_vector
```
Create a variable for the names of the days to avoid typing them again everytime:
```{r}
days_vector <- c("Monday","Tuesday","Wednesday","Thursday","Friday")
# Assign the name again
names(poker_vector) <- days_vector
names(roulette_vector) <- days_vector
```

### 2.3 Calculating vectors

You want to find out the following type of information:

-    How much has been your overall profit or loss per day of the week?
-    Have you lost money over the week in total?
-    Are you winning/losing money on poker or on roulette?

To get the answers, you have to do arithmetic calculations on vectors. If you sum two vectors, it takes the element-wise sum. 
```{r}
# For example, the following three statements are completely equivalent:
c(1,2,3) + c(4,5,6)
c(1+4,2+5,3+6)
c(5,7,9)
```
You can also do the calculations with variables that represent vectors:
```{r}
A_vector <- c(1,2,3)
B_vector <-  c(4,5,6)
total_vector <- A_vector + B_vector
total_vector # sum 2 vectors.
```
Find out the total sum of profit/lost on each day:
```{r}
total_daily <- poker_vector + roulette_vector
total_daily
```
Use `sum()` function to calculate the sum of all elements of a vector:
```{r}
total_poker <- sum(poker_vector)
total_roulette <- sum(roulette_vector)
# Total winning overall
total_week <- total_poker + total_roulette
total_week 
```
 Compare total winnings with greater sign `>`
 
```{r}
 total_poker > total_roulette 
```
You play poker better than roulette.

### 2.4 Vector selection ####

Select specific element of a vector with square brackets `[]`
```{r}
# Select Wednesday which has the index 3 in the vector
poker_wednesday <- poker_vector[3]
poker_wednesday
```
Select multiple elements from a vector with combine function in square brackets `[c()]` 
```{r}
poker_midweek <- poker_vector[c(2,3,4)]
```
Select continuous elements from a vector with `:`
```{r}
roulette_selection_vector <- roulette_vector[2:5]
```
Since the vectors are already assigned with names, we can also select names instead of index number:
```{r}
poker_vector["Monday"]
```
Select poker results for Monday, Tuesday and Wednesday:
```{r}
poker_start <- poker_vector[c("Monday","Tuesday","Wednesday")]
```
Calculate the average of the selected elements with `mean()` function
```{r}
mean(poker_start)
```

### 2.5 Selection by comparison

You can also subset the selection by using logical comparison operators:

-    `<` for less than
-    `>` for greater than
-    `<=` for less than or equal to
-    `>=` for greater than or equal to
-    ` ==` for equal to each other
-    `!=` not equal to each other

Comparison operators can be also used on vectors:
```{r}
c(4, 5, 6) > 5
```
Find out which days of winning:
```{r}
selection_vector <- poker_vector > 0
selection_vector
```
This tests for every element of the vector if the condition stated by the comparison operator is TRUE or FALSE.

Select the winning days:
```{r}
poker_winning_days <- poker_vector[selection_vector]
poker_winning_days
```
Similarly, which days did you make money on roulette?
```{r}
selection_vector <- roulette_vector > 0
roulette_winning_days <- roulette_vector[selection_vector]
roulette_winning_days
```