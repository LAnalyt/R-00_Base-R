...
# You will often work with datasets that contain different data types instead of only one. 
...
mtcars
# When you work with large data sets and data frames, it's better to develop a clear understanding of its structure and main elements. Therefore, it is often useful to show only a small part of the entire data set. 
head(mtcars) # shows the first observations of a data frame
# Similarly, tail() shows the last observations of a data frame.
tail(mtcars)

# 5.2 Structure of a data frame ####

# To get a rapid overview of your data is the structure function, use str():
...
# Applying the str() will often be the first thing that you do when receiving a new data set or data frame. It is a great way to get more insight in your dataset before diving into the real analysis.
str(mtcars) 
...
# Similar to vectors and matrices, selecting elements from a data frame can be done with square brackets []. By using a comma, you can indicate what to select from the rows and the columns respectively.
# E.g, print out diameter of Mercury (row 1, column 3):
planets_df[1,3] 
# Print out data for Mars (entire fourth row):
planets_df[4,] 
# Instead of using numerics, it is also possible to use the variable names to select columns of a data frame.
planets_df[1:3,2] 
...
# In data analysis you can sort data according to a certain variable in the dataset.
