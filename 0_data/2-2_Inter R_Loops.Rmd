...
```
Without `ctr <- ctr + 1` the code will run infinitely. Always make sure the `while` loop ends at some point.

-    `Break` statement: simply break out of the `while` loop. When R finds it, it abandons the currently active `while` loop.
```{r}
while (ctr <= 7) {
  if (ctr %% 5 == 0) {
...
```
There are some very rare situations in which severe speeding is necessary. Add a `break` in the `whil`e loop:
```{r}
speed <- 88
while (speed > 30) {
  print(paste("Your speed is", speed))
...
-    Loop over a *list*: looping over a list is as easy and convenient as looping over a vector. 
```{r}
